    ...
    return (
        <div className="fixed bottom-6 right-6 flex flex-col items-end gap-3 z-50">
            ...
            <AnimatePresence>
                {!open && idle && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="bg-white rounded-2xl rounded-br-sm shadow-xl border border-gray-100 px-4 py-3 max-w-52"
                    >
                        <p className="text-sm text-gray-500">💤 Zzz... Aa, hâlâ buradasınız!</p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {!open && proactiveVisible && !idle && (
                    <motion.div
                        ...
                        className="relative bg-white rounded-2xl rounded-br-sm shadow-xl border border-amber-100 px-4 py-3 max-w-52"
                    >
                        <p className="text-sm font-semibold text-gray-800">⚡ Güçleniyorum!</p>
                        <p className="text-xs text-gray-400 mt-1">Şarjım tam, yardıma hazırım!</p>
                        <button
                            onClick={dismiss}
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="w-80 rounded-2xl shadow-2xl overflow-hidden bg-white border border-amber-100"
                    >
                        {/* Header */}
                        ...
                        {/* Messages */}
                        <div className="px-4 py-4 flex flex-col gap-3 bg-amber-50 min-h-36">
                            <div className="flex gap-2 items-end">
                                <div className="w-6 h-6 rounded-full bg-amber-400 shrink-0" />
                        ...
                        <div className="px-3 py-3 border-t bg-white flex items-center gap-2">
                            <input
                                className="flex-1 text-sm outline-none px-3 py-2 rounded-full bg-amber-50 placeholder:text-amber-300"
            ...
            <motion.button
                ...
                className="w-16 h-16 bg-linear-to-br from-amber-400 to-orange-400 rounded-full flex items-center justify-center shadow-lg overflow-hidden"
            >
