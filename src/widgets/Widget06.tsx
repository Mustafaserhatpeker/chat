        ...
        <div className="fixed bottom-6 right-6 flex flex-col items-end gap-3 z-50">
            ...
            <AnimatePresence>
                {!open && proactiveVisible && (
                    ...
                    >
                        {/* Ana balon */}
                        <div className="bg-white rounded-2xl rounded-br-sm shadow-xl border border-sky-100 px-4 py-3 max-w-52">
                            <p className="text-sm font-semibold text-gray-800">{thoughts[thoughtIndex]}</p>
                            <p className="text-xs text-gray-400 mt-1">{greeting} Yardım etmek için buradayım!</p>
                            <button
                                onClick={dismiss}
                        ...
                        <motion.div
                            ...
                            className="absolute -bottom-2 -right-1 w-3 h-3 bg-white border border-sky-100 rounded-full shadow-sm"
                        />
                        <motion.div
                            animate={{ scale: [1, 1.2, 1] }}
                            transition={{ repeat: Infinity, duration: 2, delay: 0.3 }}
                            className="absolute -bottom-4 right-2 w-2 h-2 bg-white border border-sky-100 rounded-full shadow-sm"
                        />
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        className="w-80 rounded-2xl shadow-2xl overflow-hidden bg-white border border-sky-100"
                    >
                        {/* Header */}
                        ...
                        {/* Messages */}
                        <div className="px-4 py-4 flex flex-col gap-3 bg-sky-50 min-h-36">
                            <div className="flex gap-2 items-end">
                                <div className="w-6 h-6 rounded-full bg-sky-400 shrink-0" />
                        ...
                        <div className="px-3 py-3 border-t bg-white flex items-center gap-2">
                            <input
                                className="flex-1 text-sm outline-none px-3 py-2 rounded-full bg-sky-50 placeholder:text-sky-300"
            ...
            <motion.button
                ...
                className="w-16 h-16 bg-linear-to-br from-sky-500 to-cyan-400 rounded-full flex items-center justify-center shadow-lg overflow-hidden"
            >
